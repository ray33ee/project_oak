...
impl Command {
    ...
    pub fn action(&self, temp: & TempDir, install_archive: & mut OakRead, uninstall_archive: & mut OakWrite) -> Result<Self> {

        let mut inverses = Vec::with_capacity(self.0.len());

        for step in self.0.iter() {
            match step.action(Some(&temp), install_archive, Some(uninstall_archive)) {
                Ok(inv) => {
                    if let Some(s) = inv {
                        inverses.push(s);
                    }
    ...
    pub fn inverse(&self, uninstall_archive: & mut OakRead) {
        for step in self.0.iter() {
            step.action(None, uninstall_archive, None).unwrap();
        }
    }
